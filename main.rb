...
class Interface
  ...
  def delete_wagon_from_train
    begin
      @trains.each_with_index { |train, index| puts "#{index + 1}.#{train.number}" }
      train_selecting
      raise "У поезда нет ни одного вагона" if @trains[@train_choice].wagons == 0
    rescue StandardError => e
      puts e.message
      retry
    end
  ...
  def occupite
    ...
    if @trains[@train_choice].wagons.empty?
      raise "Нет прицепленых вагонов"
    elsif @trains[@train_choice].wagons[input].type == :cargo
  ...
  def run
    loop do
      ...
      11.Показать вагоны у поезда       12.Занять места или объем
      0.Выход"
      print "Ввод: "
      choice = gets.to_i
